...
- 메뉴에서 연결된 Wi-Fi 이름 표시 (macOS 위치 서비스 권한 필요)
...
- 연결된 Wi-Fi 이름은 macOS 위치 서비스 권한을 허용해야 표시됩니다. 권한이 없으면 메뉴에 `Connected`만 표시되며, 메뉴를 처음 열 때 권한을 요청합니다. DuoIcon은 위치 좌표를 조회하지 않습니다.
